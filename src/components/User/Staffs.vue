<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>Staff List</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card class="box-card">
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <el-button type="primary" @click="addUserFormVisible = true">Add New Staff</el-button>
        </el-col>
      </el-row>
      <!-- table -->
    ...
    <el-dialog title="Add New Staff" :visible.sync="addUserFormVisible" width="50%" @close="resetAddUserForm()">
      <el-form :model="addUserForm" :rules="addUserFormRules" ref="addUserFormRef" label-width="90px" status-icon>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitAddUserForm()">Add</el-button>
      </div>
    </el-dialog>

    <!-- dialog for editing staff -->
    <el-dialog title="Edit Staff Information" :visible.sync="editUserFormVisible" width="50%" @close="resetEditUserForm()">
      <el-form :model="editUserForm" :rules="editUserFormRules" ref="editUserFormRef" label-width="90px" status-icon>
...
export default {
    ...
    return {
      ...
      editUserFormRules: {
        ...
        role: [
          ...
          { min: 1, max: 10, message: 'the length of role is between 1 and 10', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    async getUserList() {
      var { data: res } = await this.$http.get('staffs', { params: this.queryInfo })
      // console.log(res)
    ...
    async changeState(info) {
      ...
      var { data: res } = await this.$http.put(`staffs/${info._id}/state/${info.state}`)
      // console.log(res)
      if (res.meta.status !== 200) {
        this.$message.error(res.meta.msg)
        ...
        this.$message.success('change staff state successfully')
      }
    },
    resetAddUserForm() {
    ...
    submitAddUserForm() {
      this.$refs.addUserFormRef.validate(async (valid) => {
        ...
        var { data: res } = await this.$http.post('staffs', this.addUserForm)
        if (res.meta.status === 201) {
          this.addUserFormVisible = false
          this.$message.success('add new staff successfully')
          this.getUserList()
        } else {
          this.$message.error(res.meta.msg)
        }
    ...
    async showEditUserForm(id) {
      ...
      var { data: res } = await this.$http.get('staffs/' + id)
      if (res.meta.status === 200) {
    ...
    submitEditUserForm() {
      this.$refs.editUserFormRef.validate(async (valid) => {
        ...
        var { data: res } = await this.$http.put('staffs/' + this.editUserForm._id, {
    ...
    async deleteUser(id) {
      var confirmRes = await this.$confirm('Are you sure you want to delete this staff permanently?', 'Alert:', {
      ...
      if (confirmRes === 'confirm') {
        var { data: res } = await this.$http.delete('staffs/' + id)
        if (res.meta.status === 200) {
          this.$message.success('delete staff successfully')
          this.getUserList()
        } else {
          this.$message.error(res.meta.msg)
        }
